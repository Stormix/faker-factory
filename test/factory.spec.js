...
describe("factory", function() {
  ...
  it("should create a single new instance from the factory", async function() {
    factory.register("foo", TestObject, faker => {
      return {
        foo: "bar"
      };
    });
    let object = await factory("foo").create();
    assert(object.constructor.name === "TestObject");
    assert(object.foo === "bar");
    assert(object.created === true);
  });
  it("should create many new instances from the factory and override attributes", async function() {
    factory.register("foo", TestObject, faker => {
      return {
        foo: "bar"
      };
    });
    let collection = await factory("foo", 4).create({ foo: "baz" });
    assert(collection.length === 4);
    assert(collection[0].constructor.name === "TestObject");
